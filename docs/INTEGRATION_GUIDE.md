...
## Xaloqi EDS — Zephyr RTOS, FreeRTOS, DoIP, and SOVD (v1.16.0)

| Field | Value |
|---|---|
| Stack version | 1.16.0 |
...
| 0x23 | ReadMemoryByAddress | **Implemented** | Programming session + Level 1 security required (enforced by ACL table). Parses `addressAndLengthFormatIdentifier` (1–4 byte address and size fields), validates the address range against the registered readable memory map (`uds_flash_ops_t`) before invoking `read_cb`. Request length is validated exactly against the address+size fields (no request-side data payload). |
...
| 0x37 | RequestTransferExit | **Implemented** | Requires an active transfer (NRC 0x24 requestSequenceError otherwise) and `bytes_remaining == 0` (NRC 0x31 if the tester sent fewer bytes than declared — REQ-DL-002). Optional 4-byte CRC-32 transferRequestParameterRecord, validated against the accumulated CRC if present; flushes the write buffer and invokes `flash_verify_cb`. Transfer context reset to IDLE on exit. |
| 0x38 | RequestFileTransfer | **Out of scope** | — |
| 0x3D | WriteMemoryByAddress | **Implemented** | Programming session + Level 1 security required (enforced by ACL table). Parses `addressAndLengthFormatIdentifier`, validates the address range against the registered writable memory map (`uds_flash_ops_t`) before invoking `write_cb`. Request length is validated exactly against address+size fields plus the declared data payload. |
...
**Services not listed** (e.g. 0x24 ReadScalingDataByIdentifier, 0x29 Authentication, 0x2C DynamicallyDefineDataIdentifier) are all out of scope and return NRC 0x11. 0x23 ReadMemoryByAddress and 0x3D WriteMemoryByAddress ARE implemented -- see the table above, not this exclusion list (EDS#237).
...
- **SID 0x19/0x0B — reportDTCFaultDetectionCounter: application-managed counters.** The fault detection counter for each DTC is initialised to `0x00` and must be driven by your application via `dtc_database_set_fault_counter(dtc_code, counter)`. The counter range is `0x00`–`0xFE`; `0xFF` is reserved (confirmed, excluded from the response per ISO 14229-1 §11.3.11). Only DTCs with `testFailed == 0` and `counter < 0xFF` appear in the 0x0B response. If your debouncing logic does not populate counters, the response will be an empty list (2-byte header — valid per spec).
- **SID 0x19/0x19 — reportDTCWithPermanentStatus: application-managed permanent flag.** A DTC is marked permanent via `dtc_database_set_permanent(dtc_code, true)`. Permanent DTCs are not cleared by SID 0x14 (`ClearDiagnosticInformation`); they survive until the application calls `dtc_database_set_permanent(dtc_code, false)` after a successful drive-cycle healing sequence. The `is_permanent` flag is not persisted to NVM — it is transient and must be re-evaluated by the application on each boot.
...
manifest:
  projects:
    ...
    - name: embedded-diagnostics-suite
      ...
      revision: v1.16.0            # pin to a release tag
      path: eds
```
...
{
  "sovdVersion": "1.0.0",
  "generatedBy": "Xaloqi EDS codegen v1.16.0",
