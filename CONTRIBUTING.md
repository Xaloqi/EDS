...
- Changes that break the `native_sim` CI build or cause any unit test to fail (currently 50 — see build_tests.sh, which enforces its own count via an anti-drift check)
...
# All unit tests must pass (currently 50)
bash build_tests.sh
