    ...
    uds_msg_buf_t       *resp)
{
    ...
    /* 0x23 carries no request-side data payload beyond the address/size
     * fields, so the request must be exactly fields_end bytes -- not just
     * "at least". A `<` check silently accepted trailing garbage bytes
     * (EDS#233). */
