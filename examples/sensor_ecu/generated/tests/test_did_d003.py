...
#
# Run:  pytest generated/tests/test_did_d003.py -v
# =============================================================================
...
DID_ID:         int  = 53251
DID_HEX:        str  = "0xD003"
DID_NAME:       str  = "SensorStatusBitmask"
DID_HI:         int  = 208
DID_LO:         int  = 3
DATA_LENGTH:    int  = 1
...
class TestReadSensorstatusbitmask:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xD003 (SensorStatusBitmask)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xD003 with all preconditions satisfied.
        Expected PDU: [0x62, 0xD0, 0x03, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xD003 (SensorStatusBitmask)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xD003 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xD003 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xD003 (SensorStatusBitmask) returned all-zero data — "
