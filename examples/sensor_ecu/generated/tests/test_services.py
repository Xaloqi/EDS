...
class TestReadDTCInformation:
    """SID 0x19 — DTC status reporting for 4 configured DTC(s).
    ISO 14229-1 §11.3"""
    ...
    def test_dtc_d00101_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD00101 (Ambient temperature sensor — over-range (> threshold high)) must exist in the DTC database.
        ...
        # DTC 0xD00101 = 13631745 — may be inactive (status 0x00)
        ...
        if 13631745 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD00101 (Ambient temperature sensor — over-range (> threshold high)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d00102_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD00102 (Ambient temperature sensor — under-range (< threshold low)) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD00102 = 13631746 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13631746 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD00102 (Ambient temperature sensor — under-range (< threshold low)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d00201_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD00201 (Supply voltage — over-range (> 16.0 V)) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD00201 = 13632001 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13632001 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD00201 (Supply voltage — over-range (> 16.0 V)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d00202_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD00202 (Supply voltage — under-range (< 9.0 V)) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD00202 = 13632002 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13632002 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD00202 (Supply voltage — under-range (< 9.0 V)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
...
#
# Routines under test (2):
#   0xFF00  ResetSensorCalibration  session=extended  security=1
#   0xFF01  SensorSelfTest  session=extended  security=0
# =============================================================================
...
class TestRoutineControl:
    ...
    def test_start_resetsensorcalibration(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF00 (ResetSensorCalibration) → 0x71 0x01 0xFF00.
    ...
    def test_start_sensorselftest(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xFF01 (SensorSelfTest) → 0x71 0x01 0xFF01.
    ...
    def test_resetsensorcalibration_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF00 (ResetSensorCalibration) in Default session → NRC 0x7F.
    ...
    def test_sensorselftest_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (SensorSelfTest) in Default session → NRC 0x7F.
    ...
    def test_resetsensorcalibration_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF00 (ResetSensorCalibration) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_resetsensorcalibration_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF00 (ResetSensorCalibration) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        _unlock(uds_bus, 1, aes_keys)
    ...
    def test_sensorselftest_stop_not_supported(self, uds_bus: IsoTpTransport) -> None:
        """
        stopRoutine 0xFF01 (SensorSelfTest) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
                                     255, 1]))
    ...
    def test_did_temperaturethresholdhigh_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xD010 (TemperatureThresholdHigh) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        ...
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD010 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_temperaturethresholdlow_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xD011 (TemperatureThresholdLow) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xD0,
                                      0x11]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD011 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
