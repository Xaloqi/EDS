...
#
# Run:  pytest generated/tests/test_did_d011.py -v
# =============================================================================
...
DID_ID:         int  = 53265
DID_HEX:        str  = "0xD011"
DID_NAME:       str  = "TemperatureThresholdLow"
DID_HI:         int  = 208
DID_LO:         int  = 17
DATA_LENGTH:    int  = 1
...
class TestReadTemperaturethresholdlow:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xD011 (TemperatureThresholdLow)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xD011 with all preconditions satisfied.
        Expected PDU: [0x62, 0xD0, 0x11, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xD011 (TemperatureThresholdLow)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xD011 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xD011 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xD011 (TemperatureThresholdLow) returned all-zero data — "
                "ensure handler stub is implemented."
            )

    def test_rejected_in_default_session(self, uds_bus: IsoTpTransport) -> None:
        """
        DID 0xD011 requires UDS_SESSION_EXTENDED — must be rejected in Default Session.
        ...
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD011 must not be readable in Default Session, got: {pdu.hex(' ')}"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
            NRC_CONDITIONS_NOT_CORRECT,
        ), f"Unexpected NRC 0x{pdu[2]:02X} for session-gated DID 0xD011"
...
class TestWriteTemperaturethresholdlow:
    """
    SID 0x2E WriteDataByIdentifier — DID 0xD011 (TemperatureThresholdLow)
    ...
    def test_happy_path_write_succeeds(
        ...
        """
        Write DID 0xD011 with 1 byte(s) after satisfying all preconditions.
        Expected: [0x6E, 0xD0, 0x11]
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=True)
        data = bytes([0xAA] * DATA_LENGTH)
        pdu = _write_did(uds_bus, data)

        assert pdu is not None, f"No response writing DID 0xD011"
    ...
    def test_write_read_back_roundtrip(
        ...
        """
        Write a known sentinel pattern to DID 0xD011, then read it back.
    ...
    def test_write_too_short_rejected(
        ...
        """
        pytest.skip("DID 0xD011 has data_length=1; cannot send shorter payload")
    ...
    def test_write_without_security_unlock_rejected(
        ...
        """
        Write DID 0xD011 without security unlock must return NRC 0x33.
    ...
    def test_write_wrong_session_rejected(self, uds_bus: IsoTpTransport) -> None:
        """
        Write DID 0xD011 in Default Session must be rejected.
        ...
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD011 write must fail in Default Session, got: {pdu.hex(' ')}"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
