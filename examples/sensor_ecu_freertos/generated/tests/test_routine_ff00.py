...
ROUTINE_NAME:     str  = "ResetSensorCalibration"
ROUTINE_HI:       int  = 255
ROUTINE_LO:       int  = 0
...
class TestStartResetsensorcalibration:
    """
    SID 0x31 sub-function 0x01 — startRoutine 0xFF00 (ResetSensorCalibration)
...
class TestResultsResetsensorcalibration:
    """
    SID 0x31 sub-function 0x03 — requestRoutineResults 0xFF00

    ResetSensorCalibration has ROUTINE_SUPPORT_RESULTS.
    """
...
class TestStopNotSupportedResetsensorcalibration:
    """
    SID 0x31 sub-function 0x02 — stopRoutine 0xFF00

    ResetSensorCalibration does NOT have ROUTINE_SUPPORT_STOP.
...
class TestSessionGateResetsensorcalibration:
    """
    0xFF00 (ResetSensorCalibration) requires extended session (ordinal 3).
...
class TestSecurityGateResetsensorcalibration:
    """
    0xFF00 (ResetSensorCalibration) requires security_level 1.
    ...
    def test_rejected_without_security_unlock(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xFF00 without Level 1 unlock → NRC 0x33.
        """
        if MIN_SESSION_BYTE != SESSION_DEFAULT:
...
class TestProtocolResetsensorcalibration:
    """
    Protocol boundary cases for 0xFF00 (ResetSensorCalibration).
