...
#
# Run:  pytest generated/tests/test_did_de02.py -v
# =============================================================================
...
DID_ID:         int  = 56834
DID_HEX:        str  = "0xDE02"
DID_NAME:       str  = "BMS_BalancingStateBitmask"
DID_HI:         int  = 222
DID_LO:         int  = 2
DATA_LENGTH:    int  = 1
...
class TestReadBmsbalancingstatebitmask:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xDE02 (BMS_BalancingStateBitmask)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xDE02 with all preconditions satisfied.
        Expected PDU: [0x62, 0xDE, 0x02, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xDE02 (BMS_BalancingStateBitmask)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xDE02 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xDE02 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xDE02 (BMS_BalancingStateBitmask) returned all-zero data — "
                "ensure handler stub is implemented."
            )

    def test_rejected_in_default_session(self, uds_bus: IsoTpTransport) -> None:
        """
        DID 0xDE02 requires UDS_SESSION_EXTENDED — must be rejected in Default Session.
        ...
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xDE02 must not be readable in Default Session, got: {pdu.hex(' ')}"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
            NRC_CONDITIONS_NOT_CORRECT,
        ), f"Unexpected NRC 0x{pdu[2]:02X} for session-gated DID 0xDE02"
