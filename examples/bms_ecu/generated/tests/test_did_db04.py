...
#
# Run:  pytest generated/tests/test_did_db04.py -v
# =============================================================================
...
DID_ID:         int  = 56068
DID_HEX:        str  = "0xDB04"
DID_NAME:       str  = "BMS_InsulationResistance_kOhm"
DID_HI:         int  = 219
DID_LO:         int  = 4
DATA_LENGTH:    int  = 2
...
class TestReadBmsinsulationresistancekohm:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xDB04 (BMS_InsulationResistance_kOhm)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xDB04 with all preconditions satisfied.
        Expected PDU: [0x62, 0xDB, 0x04, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xDB04 (BMS_InsulationResistance_kOhm)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xDB04 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xDB04 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xDB04 (BMS_InsulationResistance_kOhm) returned all-zero data — "
