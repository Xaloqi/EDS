...
#
# Run:  pytest generated/tests/test_did_d900.py -v
# =============================================================================
...
DID_ID:         int  = 55552
DID_HEX:        str  = "0xD900"
DID_NAME:       str  = "BMS_FaultFlagsBitmask"
DID_HI:         int  = 217
DID_LO:         int  = 0
DATA_LENGTH:    int  = 2
...
class TestReadBmsfaultflagsbitmask:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xD900 (BMS_FaultFlagsBitmask)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xD900 with all preconditions satisfied.
        Expected PDU: [0x62, 0xD9, 0x00, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xD900 (BMS_FaultFlagsBitmask)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xD900 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xD900 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xD900 (BMS_FaultFlagsBitmask) returned all-zero data — "
