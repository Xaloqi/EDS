...
class TestDiagnosticSessionControl:
    ...
    def test_enter_extended_session(self, uds_bus: IsoTpTransport) -> None:
        ...
        assert p2_ms  == 50,     f"P2server_max mismatch: {p2_ms} ms (YAML: 50)"
...
class TestReadDTCInformation:
    ...
    def test_dtc_d10100_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10100 (Cell group overvoltage — at least one cell group exceeded OV threshold) must exist in the DTC database.
        ...
        # DTC 0xD10100 = 13697280 — may be inactive (status 0x00)
        ...
        if 13697280 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10100 (Cell group overvoltage — at least one cell group exceeded OV threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10200_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10200 (Cell group undervoltage — at least one cell group dropped below UV threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10200 = 13697536 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13697536 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10200 (Cell group undervoltage — at least one cell group dropped below UV threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10300_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10300 (Cell overtemperature during discharge — cell NTC exceeded OT_discharge threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10300 = 13697792 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13697792 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10300 (Cell overtemperature during discharge — cell NTC exceeded OT_discharge threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10400_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10400 (Cell overtemperature during charge — cell NTC exceeded OT_charge threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10400 = 13698048 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13698048 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10400 (Cell overtemperature during charge — cell NTC exceeded OT_charge threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10500_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10500 (Pack overcurrent — pack current exceeded continuous current limit) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10500 = 13698304 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13698304 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10500 (Pack overcurrent — pack current exceeded continuous current limit) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10600_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10600 (Isolation fault — pack-to-chassis isolation resistance below 100 Ohm/V) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10600 = 13698560 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13698560 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10600 (Isolation fault — pack-to-chassis isolation resistance below 100 Ohm/V) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10700_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10700 (Contactor weld detected — contactor failed to open on command) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10700 = 13698816 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13698816 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10700 (Contactor weld detected — contactor failed to open on command) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10800_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10800 (Current sensor fault — sensor output out of plausibility range or open circuit) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10800 = 13699072 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13699072 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10800 (Current sensor fault — sensor output out of plausibility range or open circuit) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c00100_present_in_database(
        ...
        """
        DTC 0xC00100 (CAN communication loss — no frames received from VSC/BCM for > 500 ms) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        if 12583168 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC00100 (CAN communication loss — no frames received from VSC/BCM for > 500 ms) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_d10900_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xD10900 (BMS internal MCU fault — watchdog reset or RAM check failure on startup) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xD10900 = 13699328 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 13699328 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xD10900 (BMS internal MCU fault — watchdog reset or RAM check failure on startup) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
...
#
# Routines under test (5):
#   0xBB00  BMS_SelfTest  session=extended  security=0
#   0xBB01  BMS_ForcePassiveBalance  session=extended  security=1
#   0xBB02  BMS_ContactorFunctionalTest  session=extended  security=1
#   0xBB10  BMS_ResetSoCToFullCharge  session=programming  security=1
#   0xBB11  BMS_ClearFaultHistory  session=programming  security=1
# =============================================================================
...
class TestRoutineControl:
    ...
    def test_start_bms_selftest(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xBB00 (BMS_SelfTest) → 0x71 0x01 0xBB00.
    ...
    def test_start_bms_forcepassivebalance(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xBB01 (BMS_ForcePassiveBalance) → 0x71 0x01 0xBB01.
    ...
    def test_start_bms_contactorfunctionaltest(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xBB02 (BMS_ContactorFunctionalTest) → 0x71 0x01 0xBB02.
    ...
    def test_start_bms_resetsoctofullcharge(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xBB10 (BMS_ResetSoCToFullCharge) → 0x71 0x01 0xBB10.
    ...
    def test_start_bms_clearfaulthistory(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xBB11 (BMS_ClearFaultHistory) → 0x71 0x01 0xBB11.
    ...
    def test_bms_selftest_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB00 (BMS_SelfTest) in Default session → NRC 0x7F.
    ...
    def test_bms_forcepassivebalance_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB01 (BMS_ForcePassiveBalance) in Default session → NRC 0x7F.
    ...
    def test_bms_contactorfunctionaltest_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB02 (BMS_ContactorFunctionalTest) in Default session → NRC 0x7F.
    ...
    def test_bms_resetsoctofullcharge_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB10 (BMS_ResetSoCToFullCharge) in Default session → NRC 0x7F.
    ...
    def test_bms_clearfaulthistory_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB11 (BMS_ClearFaultHistory) in Default session → NRC 0x7F.
    ...
    def test_bms_forcepassivebalance_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB01 (BMS_ForcePassiveBalance) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_bms_contactorfunctionaltest_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB02 (BMS_ContactorFunctionalTest) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_bms_resetsoctofullcharge_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB10 (BMS_ResetSoCToFullCharge) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_bms_clearfaulthistory_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xBB11 (BMS_ClearFaultHistory) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_bms_selftest_stop_not_supported(self, uds_bus: IsoTpTransport) -> None:
        """
        stopRoutine 0xBB00 (BMS_SelfTest) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_bms_contactorfunctionaltest_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xBB02 (BMS_ContactorFunctionalTest) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_bms_resetsoctofullcharge_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xBB10 (BMS_ResetSoCToFullCharge) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_bms_clearfaulthistory_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xBB11 (BMS_ClearFaultHistory) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_did_bms_stateofhealth_04pct_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xDE01 (BMS_StateOfHealth_04pct) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xDE,
                                      0x01]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xDE01 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_bms_balancingstatebitmask_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xDE02 (BMS_BalancingStateBitmask) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xDE,
                                      0x02]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xDE02 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_bms_fullchargecapacity_wh_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xD901 (BMS_FullChargeCapacity_Wh) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xD9,
                                      0x01]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD901 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_bms_ovthreshold_mv_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xD910 (BMS_OVThreshold_mV) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xD9,
                                      0x10]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD910 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_bms_uvthreshold_mv_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xD911 (BMS_UVThreshold_mV) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xD9,
                                      0x11]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xD911 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
