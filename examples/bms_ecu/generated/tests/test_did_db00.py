...
#
# Run:  pytest generated/tests/test_did_db00.py -v
# =============================================================================
...
DID_ID:         int  = 56064
DID_HEX:        str  = "0xDB00"
DID_NAME:       str  = "BMS_PackVoltage_mV"
DID_HI:         int  = 219
DID_LO:         int  = 0
DATA_LENGTH:    int  = 4
...
class TestReadBmspackvoltagemv:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xDB00 (BMS_PackVoltage_mV)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xDB00 with all preconditions satisfied.
        Expected PDU: [0x62, 0xDB, 0x00, <4 data byte(s)>]
        Total PDU length: 7 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xDB00 (BMS_PackVoltage_mV)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xDB00 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xDB00 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xDB00 (BMS_PackVoltage_mV) returned all-zero data — "
