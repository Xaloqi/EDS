...
#
# Run:  pytest generated/tests/test_did_dd01.py -v
# =============================================================================
...
DID_ID:         int  = 56577
DID_HEX:        str  = "0xDD01"
DID_NAME:       str  = "BMS_MinCellTemperature_degC"
DID_HI:         int  = 221
DID_LO:         int  = 1
DATA_LENGTH:    int  = 1
...
class TestReadBmsmincelltemperaturedegc:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xDD01 (BMS_MinCellTemperature_degC)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xDD01 with all preconditions satisfied.
        Expected PDU: [0x62, 0xDD, 0x01, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xDD01 (BMS_MinCellTemperature_degC)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xDD01 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xDD01 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xDD01 (BMS_MinCellTemperature_degC) returned all-zero data — "
