...
#
# Run:  pytest generated/tests/test_did_dd00.py -v
# =============================================================================
...
DID_ID:         int  = 56576
DID_HEX:        str  = "0xDD00"
DID_NAME:       str  = "BMS_MaxCellTemperature_degC"
DID_HI:         int  = 221
DID_LO:         int  = 0
DATA_LENGTH:    int  = 1
...
class TestReadBmsmaxcelltemperaturedegc:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xDD00 (BMS_MaxCellTemperature_degC)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xDD00 with all preconditions satisfied.
        Expected PDU: [0x62, 0xDD, 0x00, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xDD00 (BMS_MaxCellTemperature_degC)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xDD00 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xDD00 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xDD00 (BMS_MaxCellTemperature_degC) returned all-zero data — "
