...
DID_ID:         int  = 61836
DID_HEX:        str  = "0xF18C"
DID_NAME:       str  = "ECU Serial Number"
DID_HI:         int  = 241
DID_LO:         int  = 140
DATA_LENGTH:    int  = 4
...
class TestReadEcuserialnumber:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF18C (ECU Serial Number)
    ...
    def test_happy_path_returns_correct_length(
        ...
        Expected PDU: [0x62, 0xF1, 0x8C, <4 data byte(s)>]
        Total PDU length: 7 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xF18C (ECU Serial Number)"
    ...
    def test_response_data_not_all_zero(
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF18C (ECU Serial Number) returned all-zero data — "
