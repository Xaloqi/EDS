...
#
# Run:  pytest generated/tests/test_did_2502.py -v
# =============================================================================
...
DID_ID:         int  = 9474
DID_HEX:        str  = "0x2502"
DID_NAME:       str  = "FirmwareVersion_Pending"
DID_HI:         int  = 37
DID_LO:         int  = 2
DATA_LENGTH:    int  = 4
...
class TestReadFirmwareversionpending:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2502 (FirmwareVersion_Pending)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2502 with all preconditions satisfied.
        Expected PDU: [0x62, 0x25, 0x02, <4 data byte(s)>]
        Total PDU length: 7 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2502 (FirmwareVersion_Pending)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2502 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2502 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2502 (FirmwareVersion_Pending) returned all-zero data — "
