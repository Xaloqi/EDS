...
#
# Run:  pytest generated/tests/test_did_f197.py -v
# =============================================================================
...
DID_ID:         int  = 61847
DID_HEX:        str  = "0xF197"
DID_NAME:       str  = "SystemSupplierIdentifierDataIdentifier"
DID_HI:         int  = 241
DID_LO:         int  = 151
DATA_LENGTH:    int  = 5
...
class TestReadSystemsupplieridentifierdataidentifier:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF197 (SystemSupplierIdentifierDataIdentifier)

    data_length : 5 byte(s)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xF197 with all preconditions satisfied.
        Expected PDU: [0x62, 0xF1, 0x97, <5 data byte(s)>]
        Total PDU length: 8 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xF197 (SystemSupplierIdentifierDataIdentifier)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xF197 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xF197 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF197 (SystemSupplierIdentifierDataIdentifier) returned all-zero data — "
