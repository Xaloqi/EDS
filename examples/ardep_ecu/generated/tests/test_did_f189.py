...
#
# Run:  pytest generated/tests/test_did_f189.py -v
# =============================================================================
...
DID_ID:         int  = 61833
DID_HEX:        str  = "0xF189"
DID_NAME:       str  = "ECUSoftwareVersionNumber"
DID_HI:         int  = 241
DID_LO:         int  = 137
DATA_LENGTH:    int  = 4
...
class TestReadEcusoftwareversionnumber:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF189 (ECUSoftwareVersionNumber)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xF189 with all preconditions satisfied.
        Expected PDU: [0x62, 0xF1, 0x89, <4 data byte(s)>]
        Total PDU length: 7 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xF189 (ECUSoftwareVersionNumber)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xF189 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xF189 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF189 (ECUSoftwareVersionNumber) returned all-zero data — "
