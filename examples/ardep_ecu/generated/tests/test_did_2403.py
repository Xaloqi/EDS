...
#
# Run:  pytest generated/tests/test_did_2403.py -v
# =============================================================================
...
DID_ID:         int  = 9219
DID_HEX:        str  = "0x2403"
DID_NAME:       str  = "PowerIO_OvercurrentThreshold_mA"
DID_HI:         int  = 36
DID_LO:         int  = 3
DATA_LENGTH:    int  = 2
...
class TestReadPowerioovercurrentthresholdma:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2403 (PowerIO_OvercurrentThreshold_mA)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2403 with all preconditions satisfied.
        Expected PDU: [0x62, 0x24, 0x03, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2403 (PowerIO_OvercurrentThreshold_mA)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2403 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2403 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2403 (PowerIO_OvercurrentThreshold_mA) returned all-zero data — "
                "ensure handler stub is implemented."
            )

    def test_rejected_in_default_session(self, uds_bus: IsoTpTransport) -> None:
        """
        DID 0x2403 requires UDS_SESSION_EXTENDED — must be rejected in Default Session.
        ...
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2403 must not be readable in Default Session, got: {pdu.hex(' ')}"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
            NRC_CONDITIONS_NOT_CORRECT,
        ), f"Unexpected NRC 0x{pdu[2]:02X} for session-gated DID 0x2403"
...
class TestWritePowerioovercurrentthresholdma:
    """
    SID 0x2E WriteDataByIdentifier — DID 0x2403 (PowerIO_OvercurrentThreshold_mA)
    ...
    def test_happy_path_write_succeeds(
        ...
        """
        Write DID 0x2403 with 2 byte(s) after satisfying all preconditions.
        Expected: [0x6E, 0x24, 0x03]
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=True)
        data = bytes([0xAA] * DATA_LENGTH)
        pdu = _write_did(uds_bus, data)

        assert pdu is not None, f"No response writing DID 0x2403"
    ...
    def test_write_read_back_roundtrip(
        ...
        """
        Write a known sentinel pattern to DID 0x2403, then read it back.
    ...
    def test_write_without_security_unlock_rejected(
        ...
        """
        Write DID 0x2403 without security unlock must return NRC 0x33.
    ...
    def test_write_wrong_session_rejected(self, uds_bus: IsoTpTransport) -> None:
        """
        Write DID 0x2403 in Default Session must be rejected.
        ...
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2403 write must fail in Default Session, got: {pdu.hex(' ')}"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
