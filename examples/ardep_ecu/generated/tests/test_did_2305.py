...
#
# Run:  pytest generated/tests/test_did_2305.py -v
# =============================================================================
...
DID_ID:         int  = 8965
DID_HEX:        str  = "0x2305"
DID_NAME:       str  = "ECU_LastResetReason"
DID_HI:         int  = 35
DID_LO:         int  = 5
DATA_LENGTH:    int  = 1
...
class TestReadEculastresetreason:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2305 (ECU_LastResetReason)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2305 with all preconditions satisfied.
        Expected PDU: [0x62, 0x23, 0x05, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2305 (ECU_LastResetReason)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2305 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2305 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2305 (ECU_LastResetReason) returned all-zero data — "
