...
#
# Run:  pytest generated/tests/test_did_2301.py -v
# =============================================================================
...
DID_ID:         int  = 8961
DID_HEX:        str  = "0x2301"
DID_NAME:       str  = "ECU_SupplyVoltage_mV"
DID_HI:         int  = 35
DID_LO:         int  = 1
DATA_LENGTH:    int  = 2
...
class TestReadEcusupplyvoltagemv:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2301 (ECU_SupplyVoltage_mV)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2301 with all preconditions satisfied.
        Expected PDU: [0x62, 0x23, 0x01, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2301 (ECU_SupplyVoltage_mV)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2301 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2301 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2301 (ECU_SupplyVoltage_mV) returned all-zero data — "
