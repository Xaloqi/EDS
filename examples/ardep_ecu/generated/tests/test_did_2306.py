...
#
# Run:  pytest generated/tests/test_did_2306.py -v
# =============================================================================
...
DID_ID:         int  = 8966
DID_HEX:        str  = "0x2306"
DID_NAME:       str  = "ECU_DiagnosticStackVersion"
DID_HI:         int  = 35
DID_LO:         int  = 6
DATA_LENGTH:    int  = 3
...
class TestReadEcudiagnosticstackversion:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2306 (ECU_DiagnosticStackVersion)

    data_length : 3 byte(s)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2306 with all preconditions satisfied.
        Expected PDU: [0x62, 0x23, 0x06, <3 data byte(s)>]
        Total PDU length: 6 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2306 (ECU_DiagnosticStackVersion)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2306 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2306 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2306 (ECU_DiagnosticStackVersion) returned all-zero data — "
