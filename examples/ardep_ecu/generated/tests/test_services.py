...
class TestReadDTCInformation:
    """SID 0x19 — DTC status reporting for 19 configured DTC(s).
    ISO 14229-1 §11.3"""
    ...
    def test_dtc_c00100_present_in_database(
        ...
        """
        DTC 0xC00100 (CAN bus communication loss — no frames received > 500ms) must exist in the DTC database.
        ...
        if 12583168 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC00100 (CAN bus communication loss — no frames received > 500ms) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c00200_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC00200 (CAN bus error passive — TX/RX error counters exceeded 127) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC00200 = 12583424 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12583424 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC00200 (CAN bus error passive — TX/RX error counters exceeded 127) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c00300_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC00300 (CAN bus off — transmitter disabled by bus-off recovery) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC00300 = 12583680 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12583680 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC00300 (CAN bus off — transmitter disabled by bus-off recovery) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c01000_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC01000 (LIN bus no response — no slave response within timeout) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC01000 = 12587008 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12587008 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC01000 (LIN bus no response — no slave response within timeout) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c01100_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC01100 (LIN bus framing error — break field or sync byte invalid) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC01100 = 12587264 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12587264 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC01100 (LIN bus framing error — break field or sync byte invalid) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b00100_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB00100 (Output 1 overcurrent — load current exceeded threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB00100 = 11534592 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11534592 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB00100 (Output 1 overcurrent — load current exceeded threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b00200_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB00200 (Output 2 overcurrent — load current exceeded threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB00200 = 11534848 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11534848 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB00200 (Output 2 overcurrent — load current exceeded threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b00300_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB00300 (Output 3 overcurrent — load current exceeded threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB00300 = 11535104 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11535104 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB00300 (Output 3 overcurrent — load current exceeded threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b00400_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB00400 (Output 4 overcurrent — load current exceeded threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB00400 = 11535360 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11535360 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB00400 (Output 4 overcurrent — load current exceeded threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b00500_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB00500 (Output 5 overcurrent — load current exceeded threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB00500 = 11535616 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11535616 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB00500 (Output 5 overcurrent — load current exceeded threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b00600_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB00600 (Output 6 overcurrent — load current exceeded threshold) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB00600 = 11535872 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11535872 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB00600 (Output 6 overcurrent — load current exceeded threshold) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_b01000_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xB01000 (PowerIO open load — output commanded ON but no current detected) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xB01000 = 11538432 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 11538432 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xB01000 (PowerIO open load — output commanded ON but no current detected) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0ff00_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FF00 (Supply voltage low — below 9.0V for > 500ms) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FF00 = 12648192 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12648192 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FF00 (Supply voltage low — below 9.0V for > 500ms) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0ff10_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FF10 (Supply voltage high — above 16.0V for > 100ms) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FF10 = 12648208 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12648208 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FF10 (Supply voltage high — above 16.0V for > 100ms) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0ff20_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FF20 (ECU overtemperature — internal junction > 125°C) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FF20 = 12648224 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12648224 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FF20 (ECU overtemperature — internal junction > 125°C) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0ff30_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FF30 (Watchdog reset event — firmware failed to service watchdog) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FF30 = 12648240 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12648240 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FF30 (Watchdog reset event — firmware failed to service watchdog) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0ff40_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FF40 (NVM write failure — diagnostic session statistics not persisted) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FF40 = 12648256 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12648256 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FF40 (NVM write failure — diagnostic session statistics not persisted) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0fe00_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FE00 (Firmware image verification failed — CRC mismatch) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FE00 = 12647936 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12647936 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FE00 (Firmware image verification failed — CRC mismatch) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_c0fe10_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xC0FE10 (Firmware download incomplete — transfer interrupted) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xC0FE10 = 12647952 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 12647952 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xC0FE10 (Firmware download incomplete — transfer interrupted) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
...
#   0xFF00  ECU_SelfTest  session=extended  security=0
#   0xFF01  ResetCalibrationToDefaults  session=extended  security=1
#   0xFF10  EraseApplicationFlash  session=programming  security=1
#   0xFF11  VerifyApplicationImage  session=programming  security=1
#   0xFF20  ActivateOutputChannel  session=extended  security=1
#   0xFF21  MeasureSupplyVoltage  session=extended  security=0
# =============================================================================
...
class TestRoutineControl:
    ...
    def test_start_ecu_selftest(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xFF00 (ECU_SelfTest) → 0x71 0x01 0xFF00.
    ...
    def test_start_resetcalibrationtodefaults(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF01 (ResetCalibrationToDefaults) → 0x71 0x01 0xFF01.
    ...
    def test_start_eraseapplicationflash(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF10 (EraseApplicationFlash) → 0x71 0x01 0xFF10.
    ...
    def test_start_verifyapplicationimage(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF11 (VerifyApplicationImage) → 0x71 0x01 0xFF11.
    ...
    def test_start_activateoutputchannel(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF20 (ActivateOutputChannel) → 0x71 0x01 0xFF20.
    ...
    def test_start_measuresupplyvoltage(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xFF21 (MeasureSupplyVoltage) → 0x71 0x01 0xFF21.
    ...
    def test_ecu_selftest_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF00 (ECU_SelfTest) in Default session → NRC 0x7F.
    ...
    def test_resetcalibrationtodefaults_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (ResetCalibrationToDefaults) in Default session → NRC 0x7F.
    ...
    def test_eraseapplicationflash_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF10 (EraseApplicationFlash) in Default session → NRC 0x7F.
    ...
    def test_verifyapplicationimage_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF11 (VerifyApplicationImage) in Default session → NRC 0x7F.
    ...
    def test_activateoutputchannel_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF20 (ActivateOutputChannel) in Default session → NRC 0x7F.
    ...
    def test_measuresupplyvoltage_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF21 (MeasureSupplyVoltage) in Default session → NRC 0x7F.
    ...
    def test_resetcalibrationtodefaults_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (ResetCalibrationToDefaults) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_eraseapplicationflash_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF10 (EraseApplicationFlash) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_verifyapplicationimage_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF11 (VerifyApplicationImage) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_activateoutputchannel_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF20 (ActivateOutputChannel) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_ecu_selftest_stop_not_supported(self, uds_bus: IsoTpTransport) -> None:
        """
        stopRoutine 0xFF00 (ECU_SelfTest) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_resetcalibrationtodefaults_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF01 (ResetCalibrationToDefaults) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_eraseapplicationflash_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF10 (EraseApplicationFlash) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_verifyapplicationimage_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF11 (VerifyApplicationImage) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_measuresupplyvoltage_stop_not_supported(self, uds_bus: IsoTpTransport) -> None:
        """
        stopRoutine 0xFF21 (MeasureSupplyVoltage) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
                                     255, 33]))
    ...
    def test_did_powerio_outputcontrol_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0x2010 (PowerIO_OutputControl) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0x20,
                                      0x10]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2010 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_can_busbitrate_kbps_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0x2401 (CAN_BusBitrate_kbps) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0x24,
                                      0x01]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2401 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_lin_busbaudrate_bps_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0x2402 (LIN_BusBaudrate_bps) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0x24,
                                      0x02]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2402 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_powerio_overcurrentthreshold_ma_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0x2403 (PowerIO_OvercurrentThreshold_mA) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0x24,
                                      0x03]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2403 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_watchdogtimeout_ms_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0x2404 (WatchdogTimeout_ms) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0x24,
                                      0x04]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0x2404 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
