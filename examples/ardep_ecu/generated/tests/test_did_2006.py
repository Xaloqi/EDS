...
#
# Run:  pytest generated/tests/test_did_2006.py -v
# =============================================================================
...
DID_ID:         int  = 8198
DID_HEX:        str  = "0x2006"
DID_NAME:       str  = "PowerIO_Output5_Current_mA"
DID_HI:         int  = 32
DID_LO:         int  = 6
DATA_LENGTH:    int  = 2
...
class TestReadPoweriooutput5currentma:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2006 (PowerIO_Output5_Current_mA)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2006 with all preconditions satisfied.
        Expected PDU: [0x62, 0x20, 0x06, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2006 (PowerIO_Output5_Current_mA)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2006 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2006 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2006 (PowerIO_Output5_Current_mA) returned all-zero data — "
