...
#
# Run:  pytest generated/tests/test_did_2503.py -v
# =============================================================================
...
DID_ID:         int  = 9475
DID_HEX:        str  = "0x2503"
DID_NAME:       str  = "FirmwareUpdateStatus"
DID_HI:         int  = 37
DID_LO:         int  = 3
DATA_LENGTH:    int  = 1
...
class TestReadFirmwareupdatestatus:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2503 (FirmwareUpdateStatus)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2503 with all preconditions satisfied.
        Expected PDU: [0x62, 0x25, 0x03, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2503 (FirmwareUpdateStatus)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2503 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2503 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2503 (FirmwareUpdateStatus) returned all-zero data — "
