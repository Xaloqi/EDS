...
#
# Run:  pytest generated/tests/test_did_2203.py -v
# =============================================================================
...
class TestReadCantxframecount:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2203 (CAN_TxFrameCount)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2203 with all preconditions satisfied.
        Expected PDU: [0x62, 0x22, 0x03, <4 data byte(s)>]
        Total PDU length: 7 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2203 (CAN_TxFrameCount)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2203 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2203 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2203 (CAN_TxFrameCount) returned all-zero data — "
