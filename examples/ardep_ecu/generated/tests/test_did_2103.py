...
#
# Run:  pytest generated/tests/test_did_2103.py -v
# =============================================================================
...
DID_ID:         int  = 8451
DID_HEX:        str  = "0x2103"
DID_NAME:       str  = "PowerIO_Input2_Voltage_mV"
DID_HI:         int  = 33
DID_LO:         int  = 3
DATA_LENGTH:    int  = 2
...
class TestReadPowerioinput2voltagemv:
    """
    SID 0x22 ReadDataByIdentifier — DID 0x2103 (PowerIO_Input2_Voltage_mV)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0x2103 with all preconditions satisfied.
        Expected PDU: [0x62, 0x21, 0x03, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0x2103 (PowerIO_Input2_Voltage_mV)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0x2103 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0x2103 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0x2103 (PowerIO_Input2_Voltage_mV) returned all-zero data — "
