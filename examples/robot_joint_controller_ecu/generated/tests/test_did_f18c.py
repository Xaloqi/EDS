...
#
# Run:  pytest generated/tests/test_did_f18c.py -v
# =============================================================================
...
DID_ID:         int  = 61836
DID_HEX:        str  = "0xF18C"
DID_NAME:       str  = "ControllerSerialNumber"
DID_HI:         int  = 241
DID_LO:         int  = 140
DATA_LENGTH:    int  = 8
...
class TestReadControllerserialnumber:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF18C (ControllerSerialNumber)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xF18C with all preconditions satisfied.
        Expected PDU: [0x62, 0xF1, 0x8C, <8 data byte(s)>]
        Total PDU length: 11 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xF18C (ControllerSerialNumber)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xF18C PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xF18C should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF18C (ControllerSerialNumber) returned all-zero data — "
