...
#
# Run:  pytest generated/tests/test_did_a003.py -v
# =============================================================================
...
DID_ID:         int  = 40963
DID_HEX:        str  = "0xA003"
DID_NAME:       str  = "Axis0MotorTemperature"
DID_HI:         int  = 160
DID_LO:         int  = 3
DATA_LENGTH:    int  = 1
...
class TestReadAxis0motortemperature:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xA003 (Axis0MotorTemperature)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xA003 with all preconditions satisfied.
        Expected PDU: [0x62, 0xA0, 0x03, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xA003 (Axis0MotorTemperature)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xA003 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xA003 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xA003 (Axis0MotorTemperature) returned all-zero data — "
