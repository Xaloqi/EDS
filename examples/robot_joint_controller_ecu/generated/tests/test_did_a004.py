...
#
# Run:  pytest generated/tests/test_did_a004.py -v
# =============================================================================
...
DID_ID:         int  = 40964
DID_HEX:        str  = "0xA004"
DID_NAME:       str  = "Axis0StatusBitmask"
DID_HI:         int  = 160
DID_LO:         int  = 4
DATA_LENGTH:    int  = 1
...
class TestReadAxis0statusbitmask:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xA004 (Axis0StatusBitmask)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xA004 with all preconditions satisfied.
        Expected PDU: [0x62, 0xA0, 0x04, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xA004 (Axis0StatusBitmask)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xA004 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xA004 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xA004 (Axis0StatusBitmask) returned all-zero data — "
