...
DTC_CATALOGUE = [
    {"code": 10486017, "code_hex": "0xA00101", "description": "Axis 0 motor over-temperature (> 85 °C)"},
    {"code": 10486273, "code_hex": "0xA00201", "description": "Axis 0 drive over-current (> rated peak)"},
    {"code": 10486019, "code_hex": "0xA00103", "description": "Axis 0 encoder signal loss (no valid position update > 50 ms)"},
    {"code": 10486529, "code_hex": "0xA00301", "description": "Axis 0 positive soft limit exceeded"},
    {"code": 10486530, "code_hex": "0xA00302", "description": "Axis 0 negative soft limit exceeded"},
]
...
class TestFirmwareReadDID:
    ...
    def test_read_robotserialnumber_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF190 (RobotSerialNumber) — firmware read happy path.
        Expected: RSID=0x62 DID=0xF10x90 + 17 data byte(s)
        Verifies: did_safe_read_robotserialnumber() safety wrapper + C handler
        """
    ...
    def test_read_controllerserialnumber_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF18C (ControllerSerialNumber) — firmware read happy path.
        Expected: RSID=0x62 DID=0xF10x8C + 8 data byte(s)
        Verifies: did_safe_read_controllerserialnumber() safety wrapper + C handler
        """
    ...
    def test_read_axis0position_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA000 (Axis0Position) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x00 + 4 data byte(s)
        Verifies: did_safe_read_axis0position() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[2])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 0]))
        assert pdu is not None, "No response from firmware for DID 0xA000"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        assert pdu[2] == 0, f"DID lo byte echo wrong: 0x{pdu[2]:02X}"
        expected_len = 3 + 4
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA000 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=4), got {len(pdu)}"
        )



    def test_read_axis0velocity_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA001 (Axis0Velocity) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x01 + 2 data byte(s)
        Verifies: did_safe_read_axis0velocity() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[3])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 1]))
        assert pdu is not None, "No response from firmware for DID 0xA001"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 2
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA001 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=2), got {len(pdu)}"
        )



    def test_read_axis0torque_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA002 (Axis0Torque) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x02 + 2 data byte(s)
        Verifies: did_safe_read_axis0torque() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[4])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 2]))
        assert pdu is not None, "No response from firmware for DID 0xA002"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 2
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA002 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=2), got {len(pdu)}"
        )



    def test_read_axis0motortemperature_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA003 (Axis0MotorTemperature) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x03 + 1 data byte(s)
        Verifies: did_safe_read_axis0motortemperature() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[5])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 3]))
        assert pdu is not None, "No response from firmware for DID 0xA003"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 1
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA003 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=1), got {len(pdu)}"
        )



    def test_read_axis0statusbitmask_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA004 (Axis0StatusBitmask) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x04 + 1 data byte(s)
        Verifies: did_safe_read_axis0statusbitmask() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[6])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 4]))
        assert pdu is not None, "No response from firmware for DID 0xA004"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 1
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA004 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=1), got {len(pdu)}"
        )



    def test_read_axis0positionoffset_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA010 (Axis0PositionOffset) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x10 + 4 data byte(s)
        Verifies: did_safe_read_axis0positionoffset() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[7])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 16]))
        assert pdu is not None, "No response from firmware for DID 0xA010"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 4
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA010 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=4), got {len(pdu)}"
        )

    def test_read_axis0positionoffset_session_gate_firmware(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA010 in Default Session → NRC from firmware ASIL-B safety wrapper.
        ...
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xA010 must be rejected in Default Session by firmware safety wrapper"
        )
        assert pdu[2] in (
    ...
    def test_read_axis0softlimitpositive_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA011 (Axis0SoftLimitPositive) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x11 + 4 data byte(s)
        Verifies: did_safe_read_axis0softlimitpositive() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[8])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 17]))
        assert pdu is not None, "No response from firmware for DID 0xA011"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 4
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA011 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=4), got {len(pdu)}"
        )

    def test_read_axis0softlimitpositive_session_gate_firmware(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA011 in Default Session → NRC from firmware ASIL-B safety wrapper.
        Verifies uds_safety.c session validation (step 2 of 5-step chain).
        """
        firmware_bus.request(
            bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1
        ...
        assert pdu is not None, "Expected NRC, got no response"
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xA011 must be rejected in Default Session by firmware safety wrapper"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
    ...
    def test_read_axis0softlimitnegative_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA012 (Axis0SoftLimitNegative) — firmware read happy path.
        Expected: RSID=0x62 DID=0xA00x12 + 4 data byte(s)
        Verifies: did_safe_read_axis0softlimitnegative() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[9])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 18]))
        assert pdu is not None, "No response from firmware for DID 0xA012"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 160, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        ...
        expected_len = 3 + 4
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xA012 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=4), got {len(pdu)}"
        )

    def test_read_axis0softlimitnegative_session_gate_firmware(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xA012 in Default Session → NRC from firmware ASIL-B safety wrapper.
        Verifies uds_safety.c session validation (step 2 of 5-step chain).
        """
        firmware_bus.request(
            bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1
        ...
        assert pdu is not None, "Expected NRC, got no response"
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xA012 must be rejected in Default Session by firmware safety wrapper"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
...
class TestFirmwareWriteDID:
    ...
    def test_write_axis0positionoffset_and_read_back(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        Write sentinel to DID 0xA010 then read it back from firmware.
        ...
        sentinel = bytes([(0x41 + i) & 0xFF for i in range(4)])
        pdu_w = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 16]) + sentinel
        )
        ...
        pdu_r = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 16]))
        ...
        assert readback == sentinel, (
            f"Read-back mismatch for DID 0xA010: wrote {sentinel.hex(' ')}, "
            f"got {readback.hex(' ')}"
        )

    def test_write_axis0positionoffset_security_gate(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        ...
        data = bytes([0x55] * 4)
        pdu = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 16]) + data
        )
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE
    ...
    def test_write_axis0positionoffset_wrong_length(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """Wrong data length → NRC 0x13 from firmware bounds checker."""
        _setup_for_did(firmware_bus, DID_CATALOGUE[7], for_write=True)
        wrong = bytes([0xBB] * (4 + 1))
        pdu = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 16]) + wrong
        )
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE
        assert pdu[2] == NRC_INCORRECT_MSG_LEN
    ...
    def test_write_axis0softlimitpositive_and_read_back(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        Write sentinel to DID 0xA011 then read it back from firmware.
        Proves C handler persists the value across request/response boundary.
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[8], for_write=True)
        sentinel = bytes([(0x41 + i) & 0xFF for i in range(4)])
        pdu_w = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 17]) + sentinel
        )
        assert pdu_w is not None, "No response to write"
        assert pdu_w[0] == (SID_WRITE_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Write failed: {pdu_w.hex(' ')}"
        )
        pdu_r = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 17]))
        assert pdu_r is not None
        assert pdu_r[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET)
        readback = bytes(pdu_r[3:])
        assert readback == sentinel, (
            f"Read-back mismatch for DID 0xA011: wrote {sentinel.hex(' ')}, "
            f"got {readback.hex(' ')}"
        )

    def test_write_axis0softlimitpositive_security_gate(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """Write without security unlock → NRC 0x33 from firmware ASIL-B wrapper."""
        if 3 > 1:
        ...
        data = bytes([0x55] * 4)
        pdu = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 17]) + data
        )
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE
        assert pdu[2] == NRC_SECURITY_ACCESS_DENIED, (
    ...
    def test_write_axis0softlimitpositive_wrong_length(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """Wrong data length → NRC 0x13 from firmware bounds checker."""
        _setup_for_did(firmware_bus, DID_CATALOGUE[8], for_write=True)
        wrong = bytes([0xBB] * (4 + 1))
        pdu = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 17]) + wrong
        )
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE
        assert pdu[2] == NRC_INCORRECT_MSG_LEN
    ...
    def test_write_axis0softlimitnegative_and_read_back(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        Write sentinel to DID 0xA012 then read it back from firmware.
        Proves C handler persists the value across request/response boundary.
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[9], for_write=True)
        sentinel = bytes([(0x41 + i) & 0xFF for i in range(4)])
        pdu_w = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 18]) + sentinel
        )
        assert pdu_w is not None, "No response to write"
        assert pdu_w[0] == (SID_WRITE_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Write failed: {pdu_w.hex(' ')}"
        )
        pdu_r = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 160, 18]))
        assert pdu_r is not None
        assert pdu_r[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET)
        readback = bytes(pdu_r[3:])
        assert readback == sentinel, (
            f"Read-back mismatch for DID 0xA012: wrote {sentinel.hex(' ')}, "
            f"got {readback.hex(' ')}"
        )

    def test_write_axis0softlimitnegative_security_gate(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """Write without security unlock → NRC 0x33 from firmware ASIL-B wrapper."""
        if 3 > 1:
        ...
        data = bytes([0x55] * 4)
        pdu = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 18]) + data
        )
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE
        assert pdu[2] == NRC_SECURITY_ACCESS_DENIED, (
    ...
    def test_write_axis0softlimitnegative_wrong_length(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """Wrong data length → NRC 0x13 from firmware bounds checker."""
        _setup_for_did(firmware_bus, DID_CATALOGUE[9], for_write=True)
        wrong = bytes([0xBB] * (4 + 1))
        pdu = firmware_bus.request(
            bytes([SID_WRITE_DATA_BY_ID, 160, 18]) + wrong
        )
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE
        assert pdu[2] == NRC_INCORRECT_MSG_LEN
...
class TestFirmwareIsoTp:
    ...
    def test_multiframe_robotserialnumber_response(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF190 (RobotSerialNumber) data_length=17B → 20B PDU.
    ...
    def test_multiframe_controllerserialnumber_response(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF18C (ControllerSerialNumber) data_length=8B → 11B PDU.
...
class TestFirmwareDtcInformation:
    ...
    Verifies all 5 configured DTCs are registered in firmware.
    """
    ...
    def test_dtc_a00101_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xA00101 (Axis 0 motor over-temperature (> 85 °C)) must be registered in firmware.
        ...
        0xA00101 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 1, (
                    ...
                    f"(DTC 0xA00101 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 10486017 in registered, (
                f"DTC 0xA00101 not found in firmware DTC database. "
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 1, (
                    f"DTC count {count}: DTC 0xA00101 may not be registered in firmware"
                )
    def test_dtc_a00201_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xA00201 (Axis 0 drive over-current (> rated peak)) must be registered in firmware.

        ISO 14229-1 §11.3: sub 0x01 (reportNumberOfDTCByStatusMask) reports the
        total count of registered DTCs matching the mask. With mask 0x00 (special
        case — returns ALL registered DTCs regardless of status), we can verify
        0xA00201 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 2, (
                    ...
                    f"(DTC 0xA00201 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 10486273 in registered, (
                f"DTC 0xA00201 not found in firmware DTC database. "
                f"Registered codes: {[hex(c) for c in sorted(registered)]}"
            )
        else:
            # mask=0x00 returned empty — all DTCs have status 0x00 (no faults)
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 2, (
                    f"DTC count {count}: DTC 0xA00201 may not be registered in firmware"
                )
    def test_dtc_a00103_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xA00103 (Axis 0 encoder signal loss (no valid position update > 50 ms)) must be registered in firmware.

        ISO 14229-1 §11.3: sub 0x01 (reportNumberOfDTCByStatusMask) reports the
        total count of registered DTCs matching the mask. With mask 0x00 (special
        case — returns ALL registered DTCs regardless of status), we can verify
        0xA00103 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 3, (
                    ...
                    f"(DTC 0xA00103 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 10486019 in registered, (
                f"DTC 0xA00103 not found in firmware DTC database. "
                f"Registered codes: {[hex(c) for c in sorted(registered)]}"
            )
        else:
            # mask=0x00 returned empty — all DTCs have status 0x00 (no faults)
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 3, (
                    f"DTC count {count}: DTC 0xA00103 may not be registered in firmware"
                )
    def test_dtc_a00301_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xA00301 (Axis 0 positive soft limit exceeded) must be registered in firmware.

        ISO 14229-1 §11.3: sub 0x01 (reportNumberOfDTCByStatusMask) reports the
        total count of registered DTCs matching the mask. With mask 0x00 (special
        case — returns ALL registered DTCs regardless of status), we can verify
        0xA00301 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 4, (
                    ...
                    f"(DTC 0xA00301 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 10486529 in registered, (
                f"DTC 0xA00301 not found in firmware DTC database. "
                f"Registered codes: {[hex(c) for c in sorted(registered)]}"
            )
        else:
            # mask=0x00 returned empty — all DTCs have status 0x00 (no faults)
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 4, (
                    f"DTC count {count}: DTC 0xA00301 may not be registered in firmware"
                )
    def test_dtc_a00302_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xA00302 (Axis 0 negative soft limit exceeded) must be registered in firmware.

        ISO 14229-1 §11.3: sub 0x01 (reportNumberOfDTCByStatusMask) reports the
        total count of registered DTCs matching the mask. With mask 0x00 (special
        case — returns ALL registered DTCs regardless of status), we can verify
        0xA00302 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 5, (
                    ...
                    f"(DTC 0xA00302 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 10486530 in registered, (
                f"DTC 0xA00302 not found in firmware DTC database. "
                f"Registered codes: {[hex(c) for c in sorted(registered)]}"
            )
        else:
            # mask=0x00 returned empty — all DTCs have status 0x00 (no faults)
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 5, (
                    f"DTC count {count}: DTC 0xA00302 may not be registered in firmware"
                )
...
#
# Routines under test (3 total):
#   RID 0xFF00  HomeAxis0
#     session: UDS_SESSION_EXTENDED  security: 1
#     support: start results#   RID 0xFF01  ApplyCalibration
#     session: UDS_SESSION_EXTENDED  security: 1
#     support: start results#   RID 0xFF02  ClearFaultHistory
#     session: UDS_SESSION_EXTENDED  security: 1
#     support: start# =============================================================================
...
class TestFirmwareRoutine_homeaxis0:
    """
    SID 0x31 — RID 0xFF00 HomeAxis0
    ...
    def test_start_routine_happy_path(
        ...
        """
        startRoutine 0xFF00 (HomeAxis0) in UDS_SESSION_EXTENDED session → 0x71 0x01.
    ...
    def test_security_gate_locked_nrc(
        ...
        HomeAxis0 requires security_level = 1.
    ...
    def test_stop_not_supported_nrc(
        ...
        HomeAxis0 support_flags does not include ROUTINE_SUPPORT_STOP.
    ...
    def test_request_results_happy_path(
        ...
        HomeAxis0 includes ROUTINE_SUPPORT_RESULTS.
...
class TestFirmwareRoutine_applycalibration:
    """
    SID 0x31 — RID 0xFF01 ApplyCalibration
    ...
    def test_start_routine_happy_path(
        ...
        """
        startRoutine 0xFF01 (ApplyCalibration) in UDS_SESSION_EXTENDED session → 0x71 0x01.
    ...
    def test_security_gate_locked_nrc(
        ...
        ApplyCalibration requires security_level = 1.
        Without SecurityAccess unlock, s_validate_routine_access() returns
        UDS_STATUS_ERR_SEC_NOT_UNLOCKED → NRC 0x33 securityAccessDenied.
        """
        _enter_session(firmware_bus, SESSION_EXTENDED)
    ...
    def test_stop_not_supported_nrc(
        ...
        ApplyCalibration support_flags does not include ROUTINE_SUPPORT_STOP.
        service_0x31.c: stop_cb is NULL → NRC 0x12.
        """
        _enter_session(firmware_bus, SESSION_EXTENDED)
        _unlock_level(firmware_bus, 1)
    ...
    def test_request_results_happy_path(
        ...
        ApplyCalibration includes ROUTINE_SUPPORT_RESULTS.
        Stub callback returns UDS_STATUS_OK with zero result bytes.
        Response: [0x71, 0x03, RID_hi, RID_lo]
        """
        _enter_session(firmware_bus, SESSION_EXTENDED)
...
class TestFirmwareRoutine_clearfaulthistory:
    """
    SID 0x31 — RID 0xFF02 ClearFaultHistory

    session: UDS_SESSION_EXTENDED  security_level: 1
    support: start
    All tests validate the full path through compiled firmware:
    service_0x31.c → routine_database_find(0xFF02) → callback
    """
    ...
    def test_start_routine_happy_path(
        ...
        """
        startRoutine 0xFF02 (ClearFaultHistory) in UDS_SESSION_EXTENDED session → 0x71 0x01.
    ...
    def test_session_gate_default_session_nrc(
        ...
        """
        startRoutine 0xFF02 in Default session → NRC 0x7F.
    ...
    def test_security_gate_locked_nrc(
        ...
        """
        startRoutine 0xFF02 without security unlock → NRC 0x33.

        ClearFaultHistory requires security_level = 1.
        Without SecurityAccess unlock, s_validate_routine_access() returns
        UDS_STATUS_ERR_SEC_NOT_UNLOCKED → NRC 0x33 securityAccessDenied.
        """
        _enter_session(firmware_bus, SESSION_EXTENDED)
    ...
    def test_stop_not_supported_nrc(
        ...
        """
        stopRoutine 0xFF02 → NRC 0x12 subFunctionNotSupported.

        ClearFaultHistory support_flags does not include ROUTINE_SUPPORT_STOP.
        service_0x31.c: stop_cb is NULL → NRC 0x12.
        """
        _enter_session(firmware_bus, SESSION_EXTENDED)
        _unlock_level(firmware_bus, 1)
