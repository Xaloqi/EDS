...
class TestReadDTCInformation:
    """SID 0x19 — DTC status reporting for 5 configured DTC(s).
    ISO 14229-1 §11.3"""
    ...
    def test_dtc_a00101_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xA00101 (Axis 0 motor over-temperature (> 85 °C)) must exist in the DTC database.
        ...
        # DTC 0xA00101 = 10486017 — may be inactive (status 0x00)
        ...
        if 10486017 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xA00101 (Axis 0 motor over-temperature (> 85 °C)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_a00201_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xA00201 (Axis 0 drive over-current (> rated peak)) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xA00201 = 10486273 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 10486273 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xA00201 (Axis 0 drive over-current (> rated peak)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_a00103_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xA00103 (Axis 0 encoder signal loss (no valid position update > 50 ms)) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xA00103 = 10486019 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 10486019 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xA00103 (Axis 0 encoder signal loss (no valid position update > 50 ms)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_a00301_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xA00301 (Axis 0 positive soft limit exceeded) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xA00301 = 10486529 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 10486529 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xA00301 (Axis 0 positive soft limit exceeded) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_a00302_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xA00302 (Axis 0 negative soft limit exceeded) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xA00302 = 10486530 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 10486530 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xA00302 (Axis 0 negative soft limit exceeded) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
...
#
# Routines under test (3):
#   0xFF00  HomeAxis0  session=extended  security=1
#   0xFF01  ApplyCalibration  session=extended  security=1
#   0xFF02  ClearFaultHistory  session=extended  security=1
# =============================================================================
...
class TestRoutineControl:
    ...
    def test_start_homeaxis0(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF00 (HomeAxis0) → 0x71 0x01 0xFF00.
    ...
    def test_start_applycalibration(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF01 (ApplyCalibration) → 0x71 0x01 0xFF01.
    ...
    def test_start_clearfaulthistory(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF02 (ClearFaultHistory) → 0x71 0x01 0xFF02.
    ...
    def test_homeaxis0_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF00 (HomeAxis0) in Default session → NRC 0x7F.
    ...
    def test_applycalibration_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (ApplyCalibration) in Default session → NRC 0x7F.
    ...
    def test_clearfaulthistory_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF02 (ClearFaultHistory) in Default session → NRC 0x7F.
    ...
    def test_homeaxis0_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF00 (HomeAxis0) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_applycalibration_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (ApplyCalibration) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_clearfaulthistory_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF02 (ClearFaultHistory) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_EXTENDED)
        # Deliberately do NOT unlock
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x01,
    ...
    def test_homeaxis0_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF00 (HomeAxis0) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        _unlock(uds_bus, 1, aes_keys)
    ...
    def test_applycalibration_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF01 (ApplyCalibration) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_clearfaulthistory_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF02 (ClearFaultHistory) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
    ...
    def test_did_axis0positionoffset_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xA010 (Axis0PositionOffset) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        ...
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xA010 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_axis0softlimitpositive_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xA011 (Axis0SoftLimitPositive) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xA0,
                                      0x11]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xA011 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
    def test_did_axis0softlimitnegative_rejected_in_default(self, uds_bus: IsoTpTransport) -> None:
        """DID 0xA012 (Axis0SoftLimitNegative) must be rejected in Default Session."""
        uds_bus.request(bytes([SID_DIAGNOSTIC_SESSION_CONTROL, SESSION_DEFAULT]), timeout=0.1)
        pdu = uds_bus.request(bytes([SID_READ_DATA_BY_ID,
                                      0xA0,
                                      0x12]))
        assert pdu is not None
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xA012 must be rejected in Default Session, got: {pdu.hex(' ')}"
        )
