...
DID_ID:         int  = 61840
DID_HEX:        str  = "0xF190"
DID_NAME:       str  = "RobotSerialNumber"
DID_HI:         int  = 241
DID_LO:         int  = 144
DATA_LENGTH:    int  = 17
...
class TestReadRobotserialnumber:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF190 (RobotSerialNumber)
    ...
    def test_happy_path_returns_correct_length(
        ...
        assert pdu is not None, f"No response reading DID 0xF190 (RobotSerialNumber)"
    ...
    def test_response_data_not_all_zero(
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF190 (RobotSerialNumber) returned all-zero data — "
