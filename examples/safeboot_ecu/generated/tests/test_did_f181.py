...
#
# Run:  pytest generated/tests/test_did_f181.py -v
# =============================================================================
...
DID_ID:         int  = 61825
DID_HEX:        str  = "0xF181"
DID_NAME:       str  = "ApplicationSoftwareIdentification"
DID_HI:         int  = 241
DID_LO:         int  = 129
DATA_LENGTH:    int  = 8
...
class TestReadApplicationsoftwareidentification:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF181 (ApplicationSoftwareIdentification)

    data_length : 8 byte(s)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xF181 with all preconditions satisfied.
        Expected PDU: [0x62, 0xF1, 0x81, <8 data byte(s)>]
        Total PDU length: 11 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xF181 (ApplicationSoftwareIdentification)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xF181 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xF181 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF181 (ApplicationSoftwareIdentification) returned all-zero data — "
