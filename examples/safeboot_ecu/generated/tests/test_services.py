...
class TestReadDTCInformation:
    """SID 0x19 — DTC status reporting for 3 configured DTC(s).
    ISO 14229-1 §11.3"""
    ...
    def test_dtc_f00001_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xF00001 (DFU transfer CRC mismatch — image rejected (REQ-FLASH-003)) must exist in the DTC database.
        ...
        # DTC 0xF00001 = 15728641 — may be inactive (status 0x00)
        ...
        if 15728641 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xF00001 (DFU transfer CRC mismatch — image rejected (REQ-FLASH-003)) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_f00002_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xF00002 (DFU flash erase failure) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xF00002 = 15728642 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 15728642 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xF00002 (DFU flash erase failure) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
    def test_dtc_f00003_present_in_database(
        self, uds_bus: IsoTpTransport
    ) -> None:
        """
        DTC 0xF00003 (DFU flash write failure) must exist in the DTC database.
        Validated via sub 0x02 reportDTCByStatusMask.
        """
        pdu = uds_bus.request(bytes([SID_READ_DTC_INFO, 0x02, 0xFF]))
        if pdu is None or pdu[0] == SID_NEGATIVE_RESPONSE:
        ...
        for i in range(0, len(body) - 3, 4):
            code = (body[i] << 16) | (body[i+1] << 8) | body[i+2]
            found_codes.add(code)
        # DTC 0xF00003 = 15728643 — may be inactive (status 0x00)
        # The DTC registry itself must contain it even if no fault is currently set.
        # Inform but do not fail if not present in current status mask report
        if 15728643 not in found_codes:
            import warnings
            warnings.warn(
                "DTC 0xF00003 (DFU flash write failure) not found in active DTC report. "
                "This is expected when no fault is currently set."
            )
...
#   0xFF00  CheckProgrammingPreconditions  session=extended  security=0
#   0xFF01  VerifyBootloaderIntegrity  session=programming  security=1
# =============================================================================
...
class TestRoutineControl:
    ...
    def test_start_checkprogrammingpreconditions(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xFF00 (CheckProgrammingPreconditions) → 0x71 0x01 0xFF00.
    ...
    def test_start_verifybootloaderintegrity(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        startRoutine 0xFF01 (VerifyBootloaderIntegrity) → 0x71 0x01 0xFF01.
    ...
    def test_checkprogrammingpreconditions_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF00 (CheckProgrammingPreconditions) in Default session → NRC 0x7F.
    ...
    def test_verifybootloaderintegrity_session_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (VerifyBootloaderIntegrity) in Default session → NRC 0x7F.
    ...
    def test_verifybootloaderintegrity_security_gate(self, uds_bus: IsoTpTransport) -> None:
        """
        0xFF01 (VerifyBootloaderIntegrity) without Level 1 unlock → NRC 0x33.
        """
        _session(uds_bus, SESSION_PROGRAMMING)
    ...
    def test_checkprogrammingpreconditions_stop_not_supported(self, uds_bus: IsoTpTransport) -> None:
        """
        stopRoutine 0xFF00 (CheckProgrammingPreconditions) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_EXTENDED)
    ...
    def test_verifybootloaderintegrity_stop_not_supported(self, uds_bus: IsoTpTransport, aes_keys: dict) -> None:
        """
        stopRoutine 0xFF01 (VerifyBootloaderIntegrity) → NRC 0x12 (not in support flags).
        """
        _session(uds_bus, SESSION_PROGRAMMING)
        _unlock(uds_bus, 1, aes_keys)
        pdu = uds_bus.request(bytes([SID_ROUTINE_CONTROL, 0x02,
