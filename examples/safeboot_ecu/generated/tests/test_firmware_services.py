...
DTC_CATALOGUE = [
    {"code": 15728641, "code_hex": "0xF00001", "description": "DFU transfer CRC mismatch — image rejected (REQ-FLASH-003)"},
    {"code": 15728642, "code_hex": "0xF00002", "description": "DFU flash erase failure"},
    {"code": 15728643, "code_hex": "0xF00003", "description": "DFU flash write failure"},
]
...
class TestFirmwareReadDID:
    ...
    def test_read_applicationsoftwareidentification_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF181 (ApplicationSoftwareIdentification) — firmware read happy path.
        Expected: RSID=0x62 DID=0xF10x81 + 8 data byte(s)
        Verifies: did_safe_read_applicationsoftwareidentification() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[2])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 241, 129]))
        assert pdu is not None, "No response from firmware for DID 0xF181"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 241, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        assert pdu[2] == 129, f"DID lo byte echo wrong: 0x{pdu[2]:02X}"
        expected_len = 3 + 8
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xF181 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=8), got {len(pdu)}"
        )



    def test_read_activediagnosticsession_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF186 (ActiveDiagnosticSession) — firmware read happy path.
        Expected: RSID=0x62 DID=0xF10x86 + 1 data byte(s)
        Verifies: did_safe_read_activediagnosticsession() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[3])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 241, 134]))
        assert pdu is not None, "No response from firmware for DID 0xF186"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 241, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        assert pdu[2] == 134, f"DID lo byte echo wrong: 0x{pdu[2]:02X}"
        expected_len = 3 + 1
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xF186 length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=1), got {len(pdu)}"
        )



    def test_read_systemsupplieridentifier_happy_path(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF18A (SystemSupplierIdentifier) — firmware read happy path.
        Expected: RSID=0x62 DID=0xF10x8A + 10 data byte(s)
        Verifies: did_safe_read_systemsupplieridentifier() safety wrapper + C handler
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[4])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 241, 138]))
        assert pdu is not None, "No response from firmware for DID 0xF18A"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET), (
            f"Expected positive response, got: {pdu.hex(' ')}"
        )
        assert pdu[1] == 241, f"DID hi byte echo wrong: 0x{pdu[1]:02X}"
        assert pdu[2] == 138, f"DID lo byte echo wrong: 0x{pdu[2]:02X}"
        expected_len = 3 + 10
        assert len(pdu) == expected_len, (
            f"Firmware DID 0xF18A length: expected {expected_len} "
            f"(RSID=1 + DID=2 + data=10), got {len(pdu)}"
        )
...
class TestFirmwareIsoTp:
    ...
    def test_multiframe_applicationsoftwareidentification_response(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF181 (ApplicationSoftwareIdentification) data_length=8B → 11B PDU.
        11 bytes > 7 → triggers FF+CF multi-frame from C isotp.c.
        Verifies isotp.c segments correctly and FirmwareIsoTpTransport reassembles.
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[2])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 241, 129]))
        assert pdu is not None, f"No response for multi-frame DID 0xF181"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET)
        expected_len = 3 + 8
        assert len(pdu) == expected_len, (
            f"Multi-frame reassembly error for DID 0xF181: "
            f"expected {expected_len} bytes, got {len(pdu)}"
        )
    def test_multiframe_systemsupplieridentifier_response(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DID 0xF18A (SystemSupplierIdentifier) data_length=10B → 13B PDU.
        13 bytes > 7 → triggers FF+CF multi-frame from C isotp.c.
        Verifies isotp.c segments correctly and FirmwareIsoTpTransport reassembles.
        """
        _setup_for_did(firmware_bus, DID_CATALOGUE[4])
        pdu = firmware_bus.request(bytes([SID_READ_DATA_BY_ID, 241, 138]))
        assert pdu is not None, f"No response for multi-frame DID 0xF18A"
        assert pdu[0] == (SID_READ_DATA_BY_ID + POSITIVE_RESPONSE_OFFSET)
        expected_len = 3 + 10
        assert len(pdu) == expected_len, (
            f"Multi-frame reassembly error for DID 0xF18A: "
            f"expected {expected_len} bytes, got {len(pdu)}"
        )
...
class TestFirmwareSafetyWrappers:
    ...
    def test_step5_data_length_nrc_13_firmware(
        ...
        pytest.skip("No write-capable DID in this configuration")
...
class TestFirmwareDtcInformation:
    ...
    Verifies all 3 configured DTCs are registered in firmware.
    """
    ...
    def test_dtc_f00001_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xF00001 (DFU transfer CRC mismatch — image rejected (REQ-FLASH-003)) must be registered in firmware.
        ...
        0xF00001 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 1, (
                    ...
                    f"(DTC 0xF00001 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 15728641 in registered, (
                f"DTC 0xF00001 not found in firmware DTC database. "
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 1, (
                    f"DTC count {count}: DTC 0xF00001 may not be registered in firmware"
                )
    def test_dtc_f00002_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xF00002 (DFU flash erase failure) must be registered in firmware.

        ISO 14229-1 §11.3: sub 0x01 (reportNumberOfDTCByStatusMask) reports the
        total count of registered DTCs matching the mask. With mask 0x00 (special
        case — returns ALL registered DTCs regardless of status), we can verify
        0xF00002 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 2, (
                    ...
                    f"(DTC 0xF00002 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 15728642 in registered, (
                f"DTC 0xF00002 not found in firmware DTC database. "
                f"Registered codes: {[hex(c) for c in sorted(registered)]}"
            )
        else:
            # mask=0x00 returned empty — all DTCs have status 0x00 (no faults)
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 2, (
                    f"DTC count {count}: DTC 0xF00002 may not be registered in firmware"
                )
    def test_dtc_f00003_registered(
        self, firmware_bus: FirmwareIsoTpTransport
    ) -> None:
        """
        DTC 0xF00003 (DFU flash write failure) must be registered in firmware.

        ISO 14229-1 §11.3: sub 0x01 (reportNumberOfDTCByStatusMask) reports the
        total count of registered DTCs matching the mask. With mask 0x00 (special
        case — returns ALL registered DTCs regardless of status), we can verify
        0xF00003 is in the database even when no fault is currently active.
        ...
        if pdu[0] == SID_NEGATIVE_RESPONSE:
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                assert count >= 3, (
                    ...
                    f"(DTC 0xF00003 may not be registered)"
                )
            return
        body = pdu[3:]
        if len(body) > 0 and len(body) % 4 == 0:
            registered = {(body[i]<<16)|(body[i+1]<<8)|body[i+2] for i in range(0, len(body), 4)}
            assert 15728643 in registered, (
                f"DTC 0xF00003 not found in firmware DTC database. "
                f"Registered codes: {[hex(c) for c in sorted(registered)]}"
            )
        else:
            # mask=0x00 returned empty — all DTCs have status 0x00 (no faults)
            ...
            if pdu_count is not None and pdu_count[0] == (SID_READ_DTC_INFO + POSITIVE_RESPONSE_OFFSET):
                ...
                count = struct.unpack(">H", pdu_count[4:6])[0]
                assert count >= 3, (
                    f"DTC count {count}: DTC 0xF00003 may not be registered in firmware"
                )
...
#     support: start results#   RID 0xFF01  VerifyBootloaderIntegrity
...
class TestFirmwareRoutine_checkprogrammingpreconditions:
    """
    SID 0x31 — RID 0xFF00 CheckProgrammingPreconditions
    ...
    def test_start_routine_happy_path(
        ...
        """
        startRoutine 0xFF00 (CheckProgrammingPreconditions) in UDS_SESSION_EXTENDED session → 0x71 0x01.
    ...
    def test_stop_not_supported_nrc(
        ...
        CheckProgrammingPreconditions support_flags does not include ROUTINE_SUPPORT_STOP.
    ...
    def test_request_results_happy_path(
        ...
        CheckProgrammingPreconditions includes ROUTINE_SUPPORT_RESULTS.
...
class TestFirmwareRoutine_verifybootloaderintegrity:
    """
    SID 0x31 — RID 0xFF01 VerifyBootloaderIntegrity
    ...
    def test_start_routine_happy_path(
        ...
        """
        startRoutine 0xFF01 (VerifyBootloaderIntegrity) in UDS_SESSION_PROGRAMMING session → 0x71 0x01.
    ...
    def test_security_gate_locked_nrc(
        ...
        VerifyBootloaderIntegrity requires security_level = 1.
    ...
    def test_stop_not_supported_nrc(
        ...
        VerifyBootloaderIntegrity support_flags does not include ROUTINE_SUPPORT_STOP.
        service_0x31.c: stop_cb is NULL → NRC 0x12.
        """
        _enter_session(firmware_bus, SESSION_PROGRAMMING)
        _unlock_level(firmware_bus, 1)
    ...
    def test_request_results_happy_path(
        ...
        VerifyBootloaderIntegrity includes ROUTINE_SUPPORT_RESULTS.
        Stub callback returns UDS_STATUS_OK with zero result bytes.
        Response: [0x71, 0x03, RID_hi, RID_lo]
        """
        _enter_session(firmware_bus, SESSION_PROGRAMMING)
