...
#
# Run:  pytest generated/tests/test_did_f18a.py -v
# =============================================================================
...
DID_ID:         int  = 61834
DID_HEX:        str  = "0xF18A"
DID_NAME:       str  = "SystemSupplierIdentifier"
DID_HI:         int  = 241
DID_LO:         int  = 138
DATA_LENGTH:    int  = 10
...
class TestReadSystemsupplieridentifier:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF18A (SystemSupplierIdentifier)

    data_length : 10 byte(s)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xF18A with all preconditions satisfied.
        Expected PDU: [0x62, 0xF1, 0x8A, <10 data byte(s)>]
        Total PDU length: 13 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xF18A (SystemSupplierIdentifier)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xF18A PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xF18A should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF18A (SystemSupplierIdentifier) returned all-zero data — "
