...
#
# Run:  pytest generated/tests/test_routine_cc10.py -v
# =============================================================================
...
# ── Routine under test ────────────────────────────────────────────────────────
ROUTINE_ID:       int  = 52240
ROUTINE_HEX:      str  = "0xCC10"
ROUTINE_NAME:     str  = "MC_ResolverOffsetCalibration"
ROUTINE_HI:       int  = 204
ROUTINE_LO:       int  = 16
MIN_SESSION:      int  = 2    # 1=default 2=programming 3=extended
...
class TestStartMcresolveroffsetcalibration:
    """
    SID 0x31 sub-function 0x01 — startRoutine 0xCC10 (MC_ResolverOffsetCalibration)

    min_session : programming (ordinal 2)
    security    : level 1
    """
    ...
    def test_happy_path_start(
        ...
        """
        startRoutine 0xCC10 with all preconditions satisfied.
        Expected: [0x71, 0x01, 0xCC, 0x10]
...
class TestResultsMcresolveroffsetcalibration:
    """
    SID 0x31 sub-function 0x03 — requestRoutineResults 0xCC10

    MC_ResolverOffsetCalibration has ROUTINE_SUPPORT_RESULTS.
    """
    ...
    def test_request_results(
        ...
        """
        requestRoutineResults 0xCC10 after start.
...
class TestStopNotSupportedMcresolveroffsetcalibration:
    """
    SID 0x31 sub-function 0x02 — stopRoutine 0xCC10

    MC_ResolverOffsetCalibration does NOT have ROUTINE_SUPPORT_STOP.
...
class TestSessionGateMcresolveroffsetcalibration:
    """
    0xCC10 (MC_ResolverOffsetCalibration) requires programming session (ordinal 2).
...
class TestSecurityGateMcresolveroffsetcalibration:
    """
    0xCC10 (MC_ResolverOffsetCalibration) requires security_level 1.
    Without SecurityAccess unlock → NRC 0x33 securityAccessDenied.
    """

    def test_rejected_without_security_unlock(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xCC10 without Level 1 unlock → NRC 0x33.
        """
        if MIN_SESSION_BYTE != SESSION_DEFAULT:
            _enter_session(uds_bus, MIN_SESSION_BYTE)
        # Deliberately do NOT call _unlock
        pdu = _rc(uds_bus, _SUBFN_START)
        _assert_nrc(pdu, NRC_SECURITY_ACCESS_DENIED)

# =============================================================================
# §6  Protocol boundary cases
# =============================================================================

class TestProtocolMcresolveroffsetcalibration:
    """
    Protocol boundary cases for 0xCC10 (MC_ResolverOffsetCalibration).
