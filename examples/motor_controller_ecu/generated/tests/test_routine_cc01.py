...
#
# Run:  pytest generated/tests/test_routine_cc01.py -v
# =============================================================================
...
# ── Routine under test ────────────────────────────────────────────────────────
ROUTINE_ID:       int  = 52225
ROUTINE_HEX:      str  = "0xCC01"
ROUTINE_NAME:     str  = "MC_PhaseBalanceTest"
ROUTINE_HI:       int  = 204
ROUTINE_LO:       int  = 1
...
class TestStartMcphasebalancetest:
    """
    SID 0x31 sub-function 0x01 — startRoutine 0xCC01 (MC_PhaseBalanceTest)
    ...
    def test_happy_path_start(
        ...
        """
        startRoutine 0xCC01 with all preconditions satisfied.
        Expected: [0x71, 0x01, 0xCC, 0x01]
...
class TestResultsMcphasebalancetest:
    """
    SID 0x31 sub-function 0x03 — requestRoutineResults 0xCC01

    MC_PhaseBalanceTest has ROUTINE_SUPPORT_RESULTS.
    """
    ...
    def test_request_results(
        ...
        """
        requestRoutineResults 0xCC01 after start.
...
class TestStopNotSupportedMcphasebalancetest:
    """
    SID 0x31 sub-function 0x02 — stopRoutine 0xCC01

    MC_PhaseBalanceTest does NOT have ROUTINE_SUPPORT_STOP.
...
class TestSessionGateMcphasebalancetest:
    """
    0xCC01 (MC_PhaseBalanceTest) requires extended session (ordinal 3).
...
class TestSecurityGateMcphasebalancetest:
    """
    0xCC01 (MC_PhaseBalanceTest) requires security_level 1.
    ...
    def test_rejected_without_security_unlock(self, uds_bus: IsoTpTransport) -> None:
        """
        startRoutine 0xCC01 without Level 1 unlock → NRC 0x33.
        """
        if MIN_SESSION_BYTE != SESSION_DEFAULT:
...
class TestProtocolMcphasebalancetest:
    """
    Protocol boundary cases for 0xCC01 (MC_PhaseBalanceTest).
