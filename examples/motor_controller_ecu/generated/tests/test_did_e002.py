...
#
# Run:  pytest generated/tests/test_did_e002.py -v
# =============================================================================
...
DID_ID:         int  = 57346
DID_HEX:        str  = "0xE002"
DID_NAME:       str  = "MC_TorqueDemand_01Nm"
DID_HI:         int  = 224
DID_LO:         int  = 2
DATA_LENGTH:    int  = 2
...
class TestReadMctorquedemand01nm:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xE002 (MC_TorqueDemand_01Nm)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xE002 with all preconditions satisfied.
        Expected PDU: [0x62, 0xE0, 0x02, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xE002 (MC_TorqueDemand_01Nm)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xE002 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xE002 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xE002 (MC_TorqueDemand_01Nm) returned all-zero data — "
