...
#
# Run:  pytest generated/tests/test_did_e402.py -v
# =============================================================================
...
class TestReadMcoperatingmode:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xE402 (MC_OperatingMode)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xE402 with all preconditions satisfied.
        Expected PDU: [0x62, 0xE4, 0x02, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xE402 (MC_OperatingMode)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xE402 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xE402 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xE402 (MC_OperatingMode) returned all-zero data — "
