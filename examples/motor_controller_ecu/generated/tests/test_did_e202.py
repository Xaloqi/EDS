...
#
# Run:  pytest generated/tests/test_did_e202.py -v
# =============================================================================
...
DID_ID:         int  = 57858
DID_HEX:        str  = "0xE202"
DID_NAME:       str  = "MC_DCLink_Current_100mA"
DID_HI:         int  = 226
DID_LO:         int  = 2
DATA_LENGTH:    int  = 2
...
class TestReadMcdclinkcurrent100ma:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xE202 (MC_DCLink_Current_100mA)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xE202 with all preconditions satisfied.
        Expected PDU: [0x62, 0xE2, 0x02, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xE202 (MC_DCLink_Current_100mA)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xE202 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xE202 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xE202 (MC_DCLink_Current_100mA) returned all-zero data — "
