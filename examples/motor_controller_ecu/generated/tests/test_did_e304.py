...
#
# Run:  pytest generated/tests/test_did_e304.py -v
# =============================================================================
...
DID_ID:         int  = 58116
DID_HEX:        str  = "0xE304"
DID_NAME:       str  = "MC_MotorStatorTemp_degC"
DID_HI:         int  = 227
DID_LO:         int  = 4
DATA_LENGTH:    int  = 1
...
class TestReadMcmotorstatortempdegc:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xE304 (MC_MotorStatorTemp_degC)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xE304 with all preconditions satisfied.
        Expected PDU: [0x62, 0xE3, 0x04, <1 data byte(s)>]
        Total PDU length: 4 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xE304 (MC_MotorStatorTemp_degC)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xE304 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xE304 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xE304 (MC_MotorStatorTemp_degC) returned all-zero data — "
