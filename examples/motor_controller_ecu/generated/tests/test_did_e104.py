...
#
# Run:  pytest generated/tests/test_did_e104.py -v
# =============================================================================
...
DID_ID:         int  = 57604
DID_HEX:        str  = "0xE104"
DID_NAME:       str  = "MC_Iq_Current_100mA"
DID_HI:         int  = 225
DID_LO:         int  = 4
DATA_LENGTH:    int  = 2
...
class TestReadMciqcurrent100ma:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xE104 (MC_Iq_Current_100mA)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xE104 with all preconditions satisfied.
        Expected PDU: [0x62, 0xE1, 0x04, <2 data byte(s)>]
        Total PDU length: 5 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xE104 (MC_Iq_Current_100mA)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xE104 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xE104 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xE104 (MC_Iq_Current_100mA) returned all-zero data — "
                "ensure handler stub is implemented."
            )

    def test_rejected_in_default_session(self, uds_bus: IsoTpTransport) -> None:
        """
        DID 0xE104 requires UDS_SESSION_EXTENDED — must be rejected in Default Session.
        ...
        assert pdu[0] == SID_NEGATIVE_RESPONSE, (
            f"DID 0xE104 must not be readable in Default Session, got: {pdu.hex(' ')}"
        )
        assert pdu[2] in (
            NRC_REQUEST_OUT_OF_RANGE,
            NRC_SERVICE_NOT_SUPPORTED_IN_SESSION,
            NRC_CONDITIONS_NOT_CORRECT,
        ), f"Unexpected NRC 0x{pdu[2]:02X} for session-gated DID 0xE104"
