...
class TestReadEcuserialnumber:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xF18C (ECUSerialNumber)
    ...
    def test_happy_path_returns_correct_length(
        ...
        assert pdu is not None, f"No response reading DID 0xF18C (ECUSerialNumber)"
    ...
    def test_response_data_not_all_zero(
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xF18C (ECUSerialNumber) returned all-zero data — "
