...
#
# Run:  pytest generated/tests/test_did_e201.py -v
# =============================================================================
...
DID_ID:         int  = 57857
DID_HEX:        str  = "0xE201"
DID_NAME:       str  = "MC_DCLink_Voltage_mV"
DID_HI:         int  = 226
DID_LO:         int  = 1
DATA_LENGTH:    int  = 4
...
class TestReadMcdclinkvoltagemv:
    """
    SID 0x22 ReadDataByIdentifier — DID 0xE201 (MC_DCLink_Voltage_mV)
    ...
    def test_happy_path_returns_correct_length(
        ...
        """
        Read DID 0xE201 with all preconditions satisfied.
        Expected PDU: [0x62, 0xE2, 0x01, <4 data byte(s)>]
        Total PDU length: 7 bytes.
        """
        _setup_preconditions(uds_bus, aes_keys, for_write=False)
        pdu = _read_did(uds_bus)

        assert pdu is not None, f"No response reading DID 0xE201 (MC_DCLink_Voltage_mV)"
        ...
        assert len(pdu) == expected_len, (
            f"DID 0xE201 PDU length: expected {expected_len} (RSID+DID+data), got {len(pdu)}"
        )
    ...
    def test_response_data_not_all_zero(
        ...
        """
        Data payload for DID 0xE201 should be non-zero (ECU returns real or simulated data).
        ...
        if all(b == 0 for b in data_payload):
            import warnings
            warnings.warn(
                f"DID 0xE201 (MC_DCLink_Voltage_mV) returned all-zero data — "
